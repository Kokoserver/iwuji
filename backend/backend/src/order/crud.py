...
from backend.src.order.schemas import OrderIn
...
async def get_order(user: User, orderId: str) -> Order:
    order = await Order.objects.select_related(['order_payment', "order_item_order__product",
                                                "order_item_order__product__cover_img",
                                                "shipping_address"]).get_or_none(orderId=orderId, user=user)
    if order:
        return order
    raise HTTPException(
