...
from backend.src.payment.schemas import (
    Customer,
    PaymentResponse,
    PaymentLinkData,
    PaymentMeta,
...
from backend.src.order.models import Order, OrderItem
from backend.utils.payments import generate_link, get_product_total_price, verify_payment
...
async def payment_list(user: User):
    return await Payment.objects.filter(user=user).order_by('-user_id').limit(10).all()
...
async def verify_user_payment(data: VerifyPaymentResponse, user: User):
    ...
    try:
        ...
        if not check_payment.error:
            ...
            if not get_order:
                raise HTTPException(
                    detail=f"order with user_id {data.tx_ref} does not exist",
                    status_code=status.HTTP_404_NOT_FOUND,
                )
