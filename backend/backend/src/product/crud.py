...
from backend.src.media import models
from backend.src.product.models import Product, ProductAttribute, ProductProperty, Variation
from backend.src.category.crud import Category
from backend.src.product import schemas
from backend.src.media import crud as media_crud
...
async def delete_product_gallery(productId: int) -> Product:
    get_product: Product = await Product.objects.select_related(["gallery",
                                                                 "pdf_file",
                                                                 "cover_img",
                                                                 "attribute",
                                                                 "property"]).get_or_none(id=productId)
    if not get_product:
...
async def create_product(productIn: schemas.ProductIn, cover_img: UploadFile,
                         product_pdf: UploadFile, gallery: t.List[UploadFile], request: Request) -> Message:
...
async def get_product(id: int) -> Product:
    product = await Product.objects.select_related(["gallery", "pdf_file",
                                                    "cover_img", "attribute",
                                                    "property"]).get_or_none(id=id)
    if not product:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Product not found")
    return product
...
async def update_product(productId: int,
    ...
    check_product: Product = await Product.objects.select_related(["property",
                                                                   "attribute",
                                                                   "gallery"]).get_or_none(id=productId)
    ...
    if productIn.categories:
        ...
        if get_cats:
            check_product.categories.clear()
...
async def get_products(*, limit: int, offset: int, filter: str, is_series: bool, is_assigned: bool, is_active: bool) -> \
        t.List[Product]:
...
async def update_variation(id: int, variationIn: schemas.VariationIn, cover_img: UploadFile,
                           request: Request) -> Variation:
...
async def remove_product_from_variation(dataIn: schemas.VariationProductIn) -> Message:
    ...
    if not (await Variation.objects.filter(id=dataIn.variationId).exists()):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Series not found")
    get_product_in_var = await Variation.objects.select_related("items").get_or_none(items__id=product.id,
                                                                                     id=dataIn.variationId)
