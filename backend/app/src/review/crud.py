...
async def create_review(review: schemas.ReviewIn, user: User) -> Message:
    ...
    if not await OrderItem.objects.filter(product__id=review.productId, order__user=user).exists():
        raise HTTPException(detail="user not allowed to review", status_code=status.HTTP_400_BAD_REQUEST)
    if created:
        return Message(message='Review created successfully')
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='user review is already added')
...
async def get_user_review(id: int, user: User) -> Review:
    get_review = await Review.objects.select_related('user').get_or_none(id=id, user=user)
    if not get_review:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Review does not exist')
    return get_review
...
async def get_product_review(productId: int, limit: int, offset: int) -> Review:
    ...
    get_review = await Review.objects.select_related('user').filter(product__id=productId).limit(limit).offset(offset).all()
    return get_review
