...
async def update_cart(cart: schemas.CartUpdateIn, user: User):
    get_cart_product = await Cart.objects.select_related(["product__property", "product__cover_img"]).filter(id=cart.cartId, user=user).first()
...
async def get_cart(cartId: int, user: User):
    data = await Cart.objects.select_related(["product__property", "product__cover_img"]).filter(id=cartId, user=user).first()
    return data
