...
from src.base.enum.frequent_duration import Frequent
...
from src.app.payment import schema, model
from core.settings import config
from src.lib.utils import payments
from src.app.payment.repository import payment_repo
from src.app.order.repository import order_repo
...
async def create_payment(
    ...
    total_price = payments.get_product_total_price(get_order.items)
    if total_price > 0:
        ...
        data_out: schema.IPaymentResponse = await payments.generate_link(
...
async def verify_user_payment(
    ...
    if get_payment.completed:
        raise error.BadDataError("Invalid payment details was provided")
    try:
        check_payment: schema.IPaymentVerifyOut = payments.verify_payment(
        ...
        if update_payment:
            for product in get_order.items:
                await review_repo.create(obj=dict(user=user, product=product))
            return ResponseMessage(message="payment successful")
    except Exception:
