...
async def remove_media_from_db(media_obj:Media) -> bool:
    old_media: Media = await Media.filter(pk=media_obj.id).first()
    if not old_media:
      return False
...
def convert_image_name_to_url(image_url: str, request: Request) -> str:
  image_full_url = f"{request.url_for(settings.URL_STATIC_PATH, url=image_url)}"
  return image_full_url


async def upload(media_objs:List[UploadFile], request: Request, is_pdf=False) -> List[Media]:
  if media_objs:
    ...
    media_url_list = [
        ...
        for img_name in media_name_list
    ]
...
async def update(media_objs:List[UploadFile],  old_medias: Union[List[Media],Media], request: Request, is_pdf:bool =False) -> Union[List[Media],bool]:
    if is_pdf:
    ...
    for media in old_medias:
        get_media_obj:Media = await Media.filter(pk = media.id).first()
        if get_media_obj:
            await get_media_obj.delete()
            os_file_del = os_operation.remove_path(f"{base_path}/{get_media_obj.alt}")
...
async def delete(old_medias:List[Media], is_pdf:bool = False) -> bool:
  if is_pdf:
    ...
    base_path: pydantic.DirectoryPath = settings.MEDIA_DIR
  for media in old_medias:
    get_media_obj:Media = await Media.filter(id= media.id).first()
    if get_media_obj:
      await get_media_obj.delete()
