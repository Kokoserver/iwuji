...
async def get_all_permissions(filter:str, limit:int = 10, offset:int = 0)->List[Permission]:
      return await Permission.objects.filter(name__icontains=filter).offset(offset).limit(limit)
...
async def update_permission(*, id:int, permission: schemas.PermissionIn)->Permission:
    ...
    if await Permission.objects.filter(name=permission.name):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Permission already exists")
    for key, value in permission.dict().items():
        if value != None:
            setattr(check_per, key, value)
    await check_per.save()
